...
interface HeaderProps {
  initialCategories: Array<{ id: number; name: string; slug: string }>
  initialProducts: Array<{ id: number; name: string; slug: string; price: number; images: string[] }>
}

export default function Header({ initialCategories, initialProducts }: HeaderProps) {
  ...
  const searchResults = useMemo(() => searchQuery.trim().length < 2 ? [] : initialProducts.filter((product) => product.name.toLowerCase().includes(searchQuery.toLowerCase())).slice(0, 5), [initialProducts, searchQuery])
    ...
    <header className="sticky top-0 z-40 w-full border-b border-border/70 bg-white/95 shadow-[0_10px_35px_-28px_rgba(23,33,58,.55)] backdrop-blur-xl">
      <div className="bg-brand-ink text-white">
        <div className="site-container flex min-h-9 items-center justify-center text-[0.68rem] font-bold sm:justify-between">
          ...
          <div className="hidden items-center gap-3 sm:flex">{user ? <><Link href={adminUser ? "/admin" : "/account"}>{adminUser ? 'Admin panel' : 'My account'}</Link><button onClick={async () => { await supabase.auth.signOut(); router.refresh() }} className="text-white/65 hover:text-white">Log out</button></> : <><Link href="/account/login">Log in</Link><Link href="/account/register" className="text-white/65 hover:text-white">Register</Link></>}</div>
        </div>
      </div>
      ...
      <div className="site-container flex min-h-20 items-center gap-4 py-3 lg:gap-8">
        <Sheet open={mobileOpen} onOpenChange={setMobileOpen}>
          ...
          <SheetContent side="left" className="w-[88vw] max-w-sm gap-0 bg-white p-0">
            ...
            <nav aria-label="Mobile navigation" className="flex flex-col p-3">{(adminUser ? [['Admin', '/admin'] as const, ...navItems] : navItems).map(([label, href]) => <Link key={href} href={href} onClick={() => setMobileOpen(false)} className="flex min-h-12 items-center justify-between rounded-xl px-4 font-bold text-brand-ink hover:bg-brand-cloud hover:text-primary">{label}<ArrowRight className="size-4" /></Link>)}</nav>
          </SheetContent>
        </Sheet>
      ...
      <nav aria-label="Primary navigation" className="hidden border-t border-border/70 lg:block"><div className="site-container flex h-12 items-center justify-center gap-8">{(adminUser ? [['Admin', '/admin'] as const, ...navItems] : navItems).map(([label, href]) => label === 'Shop' ? <div key={href} className="group relative"><Link href={href} aria-current={pathname.startsWith('/collections') ? 'page' : undefined} className="flex items-center gap-1 py-4 text-sm font-bold text-brand-ink hover:text-primary aria-[current=page]:text-primary">Shop collections<ChevronDown className="size-4" /></Link><div className="invisible absolute left-1/2 top-full grid w-[720px] -translate-x-1/2 grid-cols-3 gap-6 rounded-b-3xl border border-border bg-white p-7 opacity-0 shadow-2xl transition group-hover:visible group-hover:opacity-100">{[0, 5, 10].map((start, index) => <div key={start}><p className="mb-3 text-xs font-extrabold uppercase tracking-widest text-primary">{['Fresh picks', 'Bakery & pantry', 'Drinks & treats'][index]}</p>{initialCategories.slice(start, start + 5).map((category) => <Link key={category.id} href={`/collections/${category.slug}`} className="block rounded-lg py-1.5 text-xs font-semibold text-muted-foreground hover:text-brand-red">{category.name}</Link>)}</div>)}</div></div> : <Link key={href} href={href} aria-current={pathname === href ? 'page' : undefined} className="py-4 text-sm font-bold text-brand-ink hover:text-primary aria-[current=page]:text-primary">{label}</Link>)}</div></nav>
    </header>
  )
}
