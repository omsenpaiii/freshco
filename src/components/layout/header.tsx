    ...
    <header className="sticky top-0 z-40 w-full border-b border-border/70 bg-white/95 shadow-[0_10px_35px_-28px_rgba(23,33,58,.55)] backdrop-blur-xl">
      <div className="bg-brand-ink text-white">
        <div className="site-container flex min-h-9 items-center justify-center text-[0.68rem] font-bold sm:justify-between">
          <div className="hidden items-center gap-5 sm:flex"><span className="flex items-center gap-1.5"><Phone className="size-3.5 text-brand-amber" />+1 (800) 123-4567</span><span className="flex items-center gap-1.5"><MapPin className="size-3.5 text-brand-amber" />Store locator</span></div>
          <p className="tracking-wide"><span className="text-brand-amber">Fresh deal:</span> free delivery over €50</p>
      ...
      <div className="site-container flex min-h-20 items-center gap-4 py-3 lg:gap-8">
        ...
        <div className="ml-auto flex items-center gap-1 sm:gap-2">
          ...
          <Button variant="ghost" onClick={() => setCartOpen(true)} className="h-11 rounded-full px-2 sm:px-3" aria-label={`Open cart with ${cartCount} items`}><span className="relative"><ShoppingBag />{cartCount > 0 && <span className="absolute -right-2 -top-2 flex size-4 items-center justify-center rounded-full bg-brand-red text-[9px] font-extrabold text-white">{cartCount}</span>}</span><span className="hidden text-left lg:block"><small className="block text-[9px] uppercase tracking-wider text-muted-foreground">My cart</small><strong className="text-xs text-brand-ink">€{cartTotal.toFixed(2)}</strong></span></Button>
        </div>
      </div>

      <nav aria-label="Primary navigation" className="hidden border-t border-border/70 lg:block"><div className="site-container flex h-12 items-center justify-center gap-8">{navItems.map(([label, href]) => label === 'Shop' ? <div key={href} className="group relative"><Link href={href} className="flex items-center gap-1 py-4 text-sm font-bold text-brand-ink hover:text-primary">Shop collections<ChevronDown className="size-4" /></Link><div className="invisible absolute left-1/2 top-full grid w-[720px] -translate-x-1/2 grid-cols-3 gap-6 rounded-b-3xl border border-border bg-white p-7 opacity-0 shadow-2xl transition group-hover:visible group-hover:opacity-100">{[0, 5, 10].map((start, index) => <div key={start}><p className="mb-3 text-xs font-extrabold uppercase tracking-widest text-primary">{['Fresh picks', 'Bakery & pantry', 'Drinks & treats'][index]}</p>{categoriesData.slice(start, start + 5).map((category) => <Link key={category.id} href={`/collections/${category.slug}`} className="block rounded-lg py-1.5 text-xs font-semibold text-muted-foreground hover:text-brand-red">{category.name}</Link>)}</div>)}</div></div> : <Link key={href} href={href} className="py-4 text-sm font-bold text-brand-ink hover:text-primary">{label}</Link>)}</div></nav>
    </header>
  )
}
